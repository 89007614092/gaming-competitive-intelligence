...
//
// PROVIDER SEAM (Phase 4): extraction is selected by `provider` ("jina" default).
// To add a managed extractor (Firecrawl, Bright Data, etc.) implement an
// `extractViaX(canonical)` that returns { markdown, title } (or null) and branch
// on the provider inside extractArticle — no call-site changes required.
// Firecrawl is already wired: it activates only when EXTRACTOR_PROVIDER=firecrawl
// and FIRECRAWL_API_KEY is set, otherwise it falls back to Jina.
...
function createExtractor(deps = {}) {
  ...
  async function extractArticle(url, opts = {}) {
    ...
    // Route extraction by provider. Jina is the default (free, integrated).
    // Firecrawl is a drop-in when EXTRACTOR_PROVIDER=firecrawl + a key is set
    // (see the adapter contract in the module header). Both return markdown;
    // we normalise below.
    let raw, titleFromMeta = "";
    if (provider === "firecrawl" && (FIRECRAWL_API_KEY || typeof firecrawlScrape === "function")) {
      const fc = await extractViaFirecrawl(canonical);
      if (!fc) { negative.set(canonical, Date.now() + NEGATIVE_TTL_MS); return null; }
      raw = fc.markdown;
      titleFromMeta = fc.title || "";
    } else {
      if (provider === "firecrawl") {
        console.warn("[extractor] Firecrawl selected but FIRECRAWL_API_KEY missing — falling back to Jina");
      }
      raw = await extractViaJina(canonical);
      if (raw === null) return null; // negative cache already set inside
    }

    const { text, title } = parseJina(raw);
    const finalTitle = title || titleFromMeta;
    const attribution = buildAttribution(canonical, finalTitle, licenseClass);
    const result = {
      text,
      ...
      url: canonical,
      cached: false,
      via: provider === "firecrawl" ? "firecrawl" : "jina",
    };
    cache.set(canonical, { ...result, expires: Date.now() + ttlFor(licenseClass) });
    return result;
  }

  // Jina reader path — renders JS, bypasses consent walls on Jina's infra.
  // Returns the raw markdown string, or null (and records a time-bounded
  // negative cache entry) on any failure.
  async function extractViaJina(canonical) {
    ...
    try {
      ...
      // Authenticated Jina tier: an API key lifts the anonymous AbuseAlleviation
      // throttle that otherwise blocks popular domains (reuters, news.google.com,
      // …) and breaks reader retrieval. Without it the request is anonymous.
  ...
  // Firecrawl scrape path — used only when EXTRACTOR_PROVIDER=firecrawl and a
  // key (or an injected firecrawlScrape for tests) is present. Returns
  // { markdown, title } or null. Swapping in Bright Data or another provider is
  // the same shape: implement extractViaX(canonical) -> { markdown, title }.
  async function extractViaFirecrawl(canonical) {
    let fn = firecrawlScrape;
      ...
      fn = async () => {
        ...
        try {
          const res = await fetch("https://api.firecrawl.dev/v1/scrape", {
            ...
            body: JSON.stringify({ url: canonical, formats: ["markdown"] }),
            signal: controller.signal,
          });
          if (!res.ok) return null;
          const json = await res.json().catch(() => null);
          const data = json && json.data;
          const markdown = data && (data.markdown || data.content);
          if (!markdown) return null;
          return { markdown: String(markdown), title: (data.metadata && data.metadata.title) || "" };
        } catch (_) {
          return null;
        } finally {
          clearTimeout(timer);
