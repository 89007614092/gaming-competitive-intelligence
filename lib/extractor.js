...
function createExtractor(deps = {}) {
  ...
  const negative = new Set(); // canonical -> failed this session
  ...
  async function extractArticle(url, opts = {}) {
    ...
    if (negative.has(canonical)) return null;
    ...
    try {
      ...
      if (!res.ok) { negative.add(canonical); return null; }
      const ct = res.headers.get("content-type") || "";
      if (!/(text\/plain|text\/markdown|application\/json|text\/html)/i.test(ct)) {
        negative.add(canonical); return null;
      }
    ...
    if (raw.length < 120) { negative.add(canonical); return null; }
