...
async function retranslateAllKb(lang = "zh-CN") {
  if (!process.env.DEEPL_API_KEY) {
    return { keyPresent: false, deeplError: null, results: [] };
  }
  // Reset the shared provider error so this run's report reflects only the
  // failures observed while re-translating these datasets.
  ...
  for (const name of KB_DATASET_NAMES) {
    ...
    // Deep-equal (NOT reference) comparison: translateJsonDeep always builds a
    // NEW object, so `translated !== src` is always true and would make this
    // flag meaningless. JSON-equal tells us whether content actually changed —
    // e.g. a DeepL 200 response that merely echoes English must report
    // changed:false rather than the misleading always-true reference result.
    const changed = JSON.stringify(translated) !== JSON.stringify(src);
  ...
  return {
    keyPresent: true,
    deeplError: mt.getLastDeeplError ? mt.getLastDeeplError() : null,
    results,
  };
}
