...
const CPC_PARTS_RE = /^([A-HY]\d{2}[A-Z])(\d{1,4})?(?:\/(\d{2,6}))?$/;

function normaliseCpc(raw) {
  return String(raw || "").toUpperCase().replace(/\s+/g, "");
}

// OPS CQL searches CPC by the CONCATENATED symbol — `A63F13/00`, NOT the spaced
// `A63F 13/00` that Espacenet displays. This is the REVERSE of what PR #113
// concluded: the spaced form is Espacenet Smart-search syntax and OPS CQL
// rejects it with `HTTP 500 SERVER.DomainAccess`, while the concatenated form is
// accepted. Authoritative source: OPS v3.2 reference guide, Appendix 4.2
// "CQL index catalogue" — `cpc=A01B`, `cpc=/low A01B`.
//
// Hierarchy (matching a main group AND its sub-groups) is handled in buildCql
// with the `/low` RELATION qualifier — `cpc=/low A63F13/00` — NOT by appending
// `/low` to the symbol. Appending it to the value is what 500'd in #113/#114.
function formatCpc(c) {
  const m = CPC_PARTS_RE.exec(c);
  if (!m) return "";
  const [, subclass, group, sub] = m;
  let out = subclass;
  if (group) out += group;
  if (sub) out += `/${sub}`;
  return out;
...
function isCpcCode(raw) {
  // Tolerate an already-formatted value (e.g. one echoed back from `asSent`):
  // drop any existing /low and let formatCpc re-apply it by rule. Keeps the
  // function idempotent instead of rejecting its own output.
  const c = normaliseCpc(raw).replace(/\/LOW$/, "");
  return CPC_CODE_RE.test(c) ? formatCpc(c) : "";
}
...
//     "MEXICO" or "ICON".
// OPS reports its allowance in a header like "idle (4/hour)" or
// "green (15/minute)". Parsing it matters because the WINDOW is the number every
// scheduling decision depends on — and until we have read one, we are guessing
// between "15 per minute" and "15 per week", which are wildly different budgets.
function parseThrottlingControl(header) {
  const ctl = String(header || "").trim();
  if (!ctl) return null;
  const m = /([a-z]+)\s*\(\s*(\d+)\s*\/\s*([a-z]+)\s*\)/i.exec(ctl);
  if (!m) return { state: ctl, rate: null, window: null, raw: ctl };
  return {
    state: m[1].toLowerCase(),
    rate: Number(m[2]),
    window: m[3].toLowerCase(),
    raw: ctl,
  };
}
...
function buildCql(query = {}) {
  ...
  // CPC is a set — OR them together, then AND with the rest. Each code is sent
  // UNQUOTED and concatenated, with the `/low` relation qualifier so a chip
  // matches its whole sub-tree: `cpc=/low A63F13/00` = A63F13/00 AND every
  // sub-group. OPS CQL does NOT auto-post a main group the way Espacenet does,
  // so an exact `cpc=A63F13/00` would match nothing (documents are classified
  // at sub-group level, not at the /00 roll-up).
  const cpc = (Array.isArray(query.cpc) ? query.cpc : String(query.cpc || "").split(","))
    ...
    .sort();
  if (cpc.length === 1) clauses.push(`cpc=/low ${cpc[0]}`);
  else if (cpc.length > 1) clauses.push(`(${cpc.map(c => `cpc=/low ${c}`).join(" OR ")})`);
...
function normaliseSearchResult(json, limit = DEFAULT_ITEMS) {
  ...
  // The search envelope differs by constituent. With `biblio`/`abstract` OPS
  // wraps the response in `ops:biblio-search`, and THAT is where the
  // `total-result-count` attribute lives — NOT on the inner `ops:search-result`.
  // The bare `/published-data/search` returns `ops:search-result` directly.
  // Reading the count off the wrong element is why every result set reported
  // "0 matches" even though documents were present (they were still recovered
  // by the collectDocuments() fallback below, so cards rendered with total 0).
  const ENVELOPE_KEYS = [
    "ops:biblio-search", "biblioSearch",
    "ops:search-result", "search-result", "searchResult",
    "ops:register-search", "registerSearch",
  ];
  let envelope = null;
  if (root && typeof root === "object") {
    for (const k of ENVELOPE_KEYS) {
      if (root[k] !== undefined) { envelope = root[k]; break; }
    }
  }

  const totalRaw = attrOf(envelope, "@total-result-count") || attrOf(envelope, "total-result-count");
  const total = Number(String(totalRaw || "").replace(/\D/g, "")) || 0;

  // Documents sit under the envelope's `ops:search-result` child in the biblio
  // form, or directly under the envelope in the bare form.
  const inner = (envelope && (envelope["ops:search-result"] || envelope.searchResult)) || envelope || {};
  const exchange = inner["exchange-documents"] || inner.exchangeDocuments || {};
  ...
  // returns an envelope with a total of 0, and that is a successful read — not a
  // fault. Only an unrecognised payload (or one whose documents all failed to
  // parse) should be reported as unreadable.
  const sawEnvelope = !!envelope;
  return {
    patents,
...
function createEpoClient(deps = {}) {
  ...
  function noteThrottle(resp) {
    ...
    // Logged on change only: this is how we learn the real quota window for free,
    // as a by-product of calls the app was already going to make.
    if (ctl && ctl !== throttlingControl) {
      throttlingControl = ctl;
      console.info(`[epo-ops] throttling control: ${ctl}`);
    }
  ...
  //
  // `bypassBreaker` exists for explicit diagnostics. The breaker is OUR state —
  // a probe that refuses to run because the breaker tripped can never tell us
  // what tripped it, which is exactly the situation we needed to debug.
  // An explicit OPS throttle is still respected either way: that is OPS telling
  // us to wait, not our own protective state.
  async function request(path, { retryAuth = true, bypassBreaker = false } = {}) {
    if (!isConfigured()) {
    ...
    if (resp.status === 400 || resp.status === 401) {
      ...
      if (!bypassBreaker) recordFailure(err);
      throw err;
    }
  ...
  async function search(query = {}, options = {}) {
    ...
    const { patents, totalAvailable, diagnostics } = await searchCql(cql, { limit });
    return { patents, totalAvailable, diagnostics, cql };
  }

  // Runs an already-built CQL string verbatim — no code normalisation.
  // Needed by the format probe: `search()` funnels codes through isCpcCode, so
  // it cannot be used to compare the raw spellings we are trying to test.
  async function searchCql(cql, options = {}) {
    if (!isConfigured()) {
      const err = new Error("EPO OPS not configured");
      err.code = "epo_not_configured";
      throw err;
    }
    if (!cql) throw new Error("A CQL query is required");
    const limit = clampItems(options.limit);
    // The constituents matter: a BARE `/published-data/search` returns only
    // publication references (country + number + kind), with no title, applicant
    // or date. OPS only embeds `bibliographic-data` when you ask for it, which
    // is why the first version of this rendered zero usable cards. Requesting
    // `abstract` in the same call also removes the need for the per-hit
    // abstract round-trips entirely (they were up to 10 extra calls).
    const url = `/published-data/search/abstract,biblio?q=${encodeURIComponent(cql)}&Range=1-${limit}`;
    // `bypassBreaker` lets a diagnostic probe run even while OUR circuit is
    // open: a probe that refuses to run because the breaker tripped can never
    // tell us what tripped it. A bypassed call also records no failure, so
    // testing a known-bad spelling (e.g. `/low`) cannot poison the breaker.
    const json = await request(url, { bypassBreaker: options.bypassBreaker === true });
  ...
  return {
    isConfigured, search, searchCql, biblio, fetchAbstract, enrichWithAbstracts,
