...
// OPS indexes CPC in the SPACED form — the same form it returns in biblio data:
// `A63F 13/00`, not `A63F13/00`. The EPO's own Espacenet/OPS release notes write
// it as "C08F 220" and state that main-group level (C08, C08F, C08F 220) is
// auto-posted so all sub-levels are searched automatically, while SUBGROUP level
// needs a trailing `/low` to include children (their example: C08F220/26/low).
//
// Sending the un-spaced form matched nothing at all, which is why every
// group-level code reported zero results.
function formatCpc(c) {
  ...
  if (group) out += ` ${group}`;
  if (sub) out += `/${sub}`;
  // `/low` is deliberately NOT appended, even though the EPO's Espacenet notes
  // say subgroup level needs it to include children. OPS rejected it with
  // `HTTP 500 SERVER.DomainAccess`, taking the whole Patents tab down. That
  // syntax is Espacenet Smart search; OPS CQL does not accept it.
  // See probe-cpc-format — if OPS ever supports hierarchy, this is where to
  // add it back.
  return out;
}
