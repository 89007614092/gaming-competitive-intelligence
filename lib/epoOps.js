...
function formatCpc(c) {
  ...
  if (sub) {
    out += `/${sub}`;
    // `/00` is a main group and is auto-posted; anything deeper is a subgroup
    // and needs /low or it matches only documents classified at exactly that
    // level (e.g. A63F 13/67 would silently miss A63F 13/69).
    if (!/^0+$/.test(sub)) out += "/low";
  }
  return out;
}
