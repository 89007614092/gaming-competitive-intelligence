 ...
 *     (api.deepl.com). The endpoint can be overridden with DEEPL_ENDPOINT.
 *     Auth is sent BOTH as the legacy `auth_key` form param AND as the
 *     documented `DeepL-Auth-Key` Authorization header, so the call works
 *     regardless of which method the account expects.
...
// ---- DeepL ----------------------------------------------------------------
// The most recent DeepL failure, surfaced by /healthz (deeplError) and the
// retranslate-kb diagnostic so a broken / expired / over-quota key is
// immediately diagnosable (e.g. "DeepL HTTP 456: Quota exceeded") instead of
// silently degrading to English. null = no failure observed this process.
let _lastDeeplError = null;

function getLastDeeplError() {
  return _lastDeeplError;
}
function clearLastDeeplError() {
  _lastDeeplError = null;
}

// Resolve the DeepL base URL.
//   - DEEPL_ENDPOINT (optional) pins the exact base, e.g. a regional endpoint
//     or a proxy. This lets the operator set the endpoint explicitly instead
//     of relying on suffix inference.
//   - Otherwise derive it from the key suffix, exactly as DeepL's docs require:
//     Free keys end in ":fx" and hit https://api-free.deepl.com; every other
//     key is Pro and hits https://api.deepl.com. Contacting the wrong endpoint
//     makes auth fail (403) even with a valid key, which is the classic
//     "key set but nothing translates" symptom.
function deeplBaseUrl() {
  const override = (process.env.DEEPL_ENDPOINT || '').trim();
...
async function deeplTranslateBatch(items, { target = 'ZH', source = 'EN' } = {}) {
  ...
  params.set('auth_key', key); // legacy form param — still accepted by DeepL
  // Omitting source_lang lets DeepL auto-detect it. Patent titles arrive in
  // whatever language the filing office used, so callers often do not know the
  // source — and guessing "EN" mangles non-English input.
  ...
  let resp;
  try {
    resp = await fetch(`${base}/v2/translate`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
        // DeepL's current, recommended authentication: the DeepL-Auth-Key
        // Authorization header. Sent in addition to the legacy auth_key form
        // param so the call succeeds regardless of which method the account
        // expects (both are officially supported by DeepL).
        Authorization: `DeepL-Auth-Key ${key}`,
      },
      body: params.toString(),
    });
  } catch (e) {
    _lastDeeplError = `DeepL network error: ${e.message}`;
    throw e;
  }
  if (!resp.ok) {
    const txt = await resp.text().catch(() => '');
    _lastDeeplError = `DeepL HTTP ${resp.status}: ${txt.slice(0, 200)}`;
    throw new Error(_lastDeeplError);
  }
...
async function googleTranslateBatch(items, { target = 'zh-CN', source = 'en' } = {}) {
  ...
  const resp = await fetch(
    `https://translation.googleapis.com/language/translate/v2?key=${encodeURIComponent(key)}`,
    {
      ...
      body: JSON.stringify({ q: items.map((it) => it.masked), target, format: 'text', ...(source ? { source } : {}) }),
    }
  );
  if (!resp.ok) {
    const txt = await resp.text().catch(() => '');
