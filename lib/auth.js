...
// Async role lookup (v1.2): `ADMIN_EMAILS` env is the break-glass authority,
// otherwise consult the `allowed_emails.is_admin` column so admin can be managed
// in Supabase. DB errors fail CLOSED (no admin promotion) so a broken lookup
// never silently grants admin. Returns "admin" | "user".
async function roleForEmailAsync(email) {
  const e = String(email || "").trim().toLowerCase();
  if (!e) return "user";
  if (ADMIN_EMAILS.includes(e)) return "admin"; // env always authoritative
  if (!_pool) return "user";
  try {
    const { rows } = await _pool.query(
      "SELECT is_admin FROM allowed_emails WHERE email = $1 LIMIT 1",
      [e]
    );
    return rows.length && rows[0].is_admin ? "admin" : "user";
  } catch (err) {
    console.warn("[auth] is_admin lookup failed:", err.message);
    return "user";
  }
}

// ---- Allow-list pool (Supabase Postgres `allowed_emails` table) -------------
// Hybrid management: an email is allowed if it is in ALLOWED_EMAILS (env, the
// break-glass bootstrap) OR present in the `allowed_emails` table. The pool is
// injected by server.js at boot (reusing its existing connection) so we don't
// open a second pool. If no pool is available, the env list is authoritative
// and the table is simply not consulted.
let _pool = null;
function __setPool(p) { _pool = p; }

// Async allow-list check: env fast-path, then the optional Supabase table.
// DB errors fail CLOSED (treat as not allowed) so a broken lookup never opens
// the app to an unlisted address.
async function emailAllowedAsync(email) {
  if (emailAllowed(email)) return true; // env is always authoritative
  if (!_pool) return false;             // no DB → env only
  try {
    const { rows } = await _pool.query(
    ...
    console.warn("[auth] allowed_emails lookup failed:", e.message);
    return false;
  }
}
...
// Sends a magic link (or, when a password is supplied, signs in directly) only
// if the email is on the allow-list (env OR the allowed_emails table).
async function sendMagicLink(req, res) {
  ...
  const allowed = await emailAllowedAsync(email);
  if (!allowed) {
  ...
  if (result.error) {
    console.warn("[auth] login error:", result.error.message);
    // Supabase's built-in email provider caps auth emails at ~2/hour,
    // project-wide — so a *different* recipient is still blocked. The magic-link
    // path (signInWithOtp) hits this; password login sends no email and is
    // unaffected. Surface a clear, actionable message instead of the raw
    // "email rate limit exceeded" string.
    const isEmailRateLimit =
      result.error.status === 429 ||
      /email rate limit|over_email_send_rate_limit/i.test(result.error.message || "");
    if (isEmailRateLimit && !password) {
      return res.status(429).json({
        code: "email_rate_limit",
        error:
          "Email send limit reached. Supabase's built-in provider caps auth emails at ~2/hour, project-wide — so a different address is still blocked. Use password login (no email is sent) or wait about an hour. For production magic links, configure a custom SMTP provider in Supabase → Authentication → SMTP Settings.",
      });
    }
    return res.status(result.error.status || 400).json({ error: result.error.message });
  }
  // Magic-link: tell the UI to wait for the email. Password: UI redirects in.
  return res.json({ ok: true, sent: !password, redirect: password ? "/" : null });
}
...
async function getSession(req, res) {
  ...
  return res.json({ user: { email: user.email, role: await roleForEmailAsync(user.email), id: user.id } });
}
...
function authGate(req, res, next) {
  ...
  return getUserFromRequest(req, res)
    .then(async (user) => {
      if (user) {
        req.user = { email: user.email, role: await roleForEmailAsync(user.email), id: user.id };
