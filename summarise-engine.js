...
function formatContext(evidence, question) {
  return evidence.map(item => {
    // User-attached [S#] and enriched web [W#] carry fuller content, so give
    // them a larger excerpt budget; application [A#] keeps the tighter one to
    // protect the context window.
    const budget = item.sourceType === "user"
      ? [4, 1400]
      : item.sourceType === "internet"
        ? [3, 900]
        : [2, 620];
    return `[${item.id}] ${item.dataset} — ${item.title}${item.section ? ` (${item.section})` : ""}\n${relevantExcerpt(item, question, budget[0], budget[1], item.title)}`;
  }).join("\n\n");
}
...
async function nudgeForUserSources(baseMessages, currentAnswer, evidence, userEvidenceIds, lang = "en") {
  const nudgeMessages = [
    ...baseMessages,
    {
      role: "user",
      content:
        "Ensure every [S#] source the user attached is reflected in your answer — you may cite it " +
  ...
  try {
    ...
    // the residual gap).
    const recovered = userEvidenceIds.some(id => candidate.includes(`[${id}]`));
...
function applyLanguageInstruction(systemPrompt, lang = "en") {
  ...
  return (
    ...
    "4. 紧扣证据作答，引用标记须落在真正支撑该论断的出处上。5. 每一段正文与每一条要点都必须带至少一个引用标记；无证据支撑的论断要么删去，要么明确说明缺乏证据，不得裸述，也不得为凑引用而引用无关的出处。"
  );
}
...
// Observability only — how much of an answer makes claims with no citation.
// Logged, never enforced: a sudden jump means the model is drifting away from
// the citation discipline the prompt asks for, which is otherwise invisible
// until someone reads the answer closely.
function uncitedSegments(answer) {
  return String(answer || "")
    .split("\n")
    .map(line => line.trim())
    .filter(Boolean)
    .filter(line => !/^#{1,6}\s/.test(line))      // not a heading
    .filter(line => /[.!?。！？]$/.test(line))      // a complete segment
    .filter(line => !/\[[AWST]\d+\]/.test(line)); // and no citation in it
}

function evaluateCitationGate(answer, evidence) {
...
function webResultRelevance(question, results, limit = 5) {
  ...
  return (results || [])
    .map(item => {
      ...
      // Weight each matching bigram by how DISTINGUISHING it is. Equal weighting
      // let a page full of generic terms (人工智能) outrank one that actually
      // names the subject (腾讯) — see lib/text-cjk.js bigramWeight().
      let cjkScore = 0;
      for (const bg of overlapCjk) cjkScore += textCjk.bigramWeight(bg);
      score += Math.min(cjkScore, 6) * 6;
...
function buildExtractiveAnswer(question, evidence, style = "full", lang = "en") {
  ...
  if (appItems.length) {
    ...
    for (const item of appItems) {
      const excerpt = relevantExcerpt(item, question, 2, 620, item.title);
      if (!excerpt) continue;
